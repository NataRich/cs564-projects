...
void test_clock_algo_1(File &file6);
void test_clock_algo_2(File &file6);
void test_clock_algo_3(File &file6);
void test_clock_algo_4(File &file6);

// Calls above clock algo tests
void testBufMgrClock();

void test_page_1(File &file7);
void test_page_2(File &file8);
...
int main() {
  ...
  std::cout << "\n\nNext test suite: clock algorithm tests\n\n";

  // This function specifically tests buffer manager's clock algorithm
  testBufMgrClock();

  std::cout << "\n\nNext test suite: unPin & allocate Page\n\n";
...
void testBufMgrClock() {
  ...
  // Create dummy files
  const std::string filename6 = "algo_test.1";
...

  {
    ...
    // Tests if the clock algorithm is implemented correctly
    test_clock_algo_1(file6);
    test_clock_algo_2(file6);
    test_clock_algo_3(file6);
    test_clock_algo_4(file6);
...
void testBufMgrPage() {
  // Create dummy files
  const std::string filename7 = "page_test.1";
...
void test_clock_algo_1(File &file6) {
  PageId pageId;
  bufMgr->allocPage(file6, pageId, page);
  bufMgr->unPinPage(file6, pageId, false);
  if (bufMgr->bufPool[0].page_number() != pageId) {
    bufMgr->flushFile(file6);
    PRINT_ERROR("ERROR :: Clock algo didn't allocate the correct frame.");
  }

  std::cout << "Algo test 1 passed" << std::endl;
...
void test_clock_algo_2(File &file6) {
  ...
  bufMgr->allocPage(file6, pageId, page);
  bufMgr->unPinPage(file6, pageId, false);
  if (bufMgr->bufPool[1].page_number() != pageId) {
    bufMgr->flushFile(file6);
    PRINT_ERROR("ERROR :: Clock algo didn't allocate the correct frame.");
  }

  std::cout << "Algo test 2 passed" << std::endl;
...
void test_clock_algo_3(File &file6) {
  ...
  for (i = 10; i < num; i++) {
    bufMgr->unPinPage(file6, pid[i], false);
  }

  bufMgr->allocPage(file6, pid[10], page);
  if (bufMgr->bufPool[12].page_number() != pid[10]) {
    for (i = 0; i < num; i++) {
      bufMgr->unPinPage(file6, pid[i], false);
    }
    bufMgr->flushFile(file6);
    PRINT_ERROR(
        "ERROR :: Clock algo didn't allocate the least recently used frame.");
  }

  std::cout << "Algo test 3 passed" << std::endl;
...
void test_clock_algo_4(File &file6) {
  bufMgr->readPage(file6, 1, page);
  if (bufMgr->bufPool[13].page_number() != 1) {
    bufMgr->unPinPage(file6, 1, false);
    bufMgr->flushFile(file6);
    PRINT_ERROR(
        "ERROR :: Clock algo didn't allocate the least recently used frame.");
  }

  std::cout << "Algo test 4 passed" << std::endl;
...
void test_page_1(File &file7) {
  for (i = 0; i < num; i++) {
    bufMgr->allocPage(file7, pid[i], page);
  }
  bufMgr->unPinPage(file7, 1, page);

  try {
    bufMgr->unPinPage(file7, 1, page);
    PRINT_ERROR("ERROR :: first PageNotPinnedException not raised.");
  } catch (const PageNotPinnedException &e) {
  }

  try {
    bufMgr->unPinPage(file7, 0, page);
    bufMgr->unPinPage(file7, 1, page);
    PRINT_ERROR("ERROR :: second PageNotPinnedException not raised.");
  } catch (const PageNotPinnedException &e) {
  }

  bufMgr->unPinPage(file7, num, page);

  for (i = 2; i < num; i++) bufMgr->unPinPage(file7, i, true);
  bufMgr->flushFile(file7);
}

void test_page_2(File &file8) {
  try {
    file8.open("page_test.2");
  } catch (const FileNotFoundException &e) {
  }
}
